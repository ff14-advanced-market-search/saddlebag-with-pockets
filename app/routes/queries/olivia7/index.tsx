...
import { setOlivia7 } from '~/redux/reducers/queriesSlice'
...
const Index = () => {
  ...
  const Olivia7 = useTypedSelector((state) => state.queries.Olivia7)

  const dispatch = useDispatch()

  useEffect(() => {
    if (results && results.data) {
      dispatch(setOlivia7(results.data))
    }
  }, [results, dispatch])
    ...
    <main className="flex-1">
      <div className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
          ...
          {Olivia7 && !results && (
            <PreviousResultsLink to="/queries/previous-search?query=Olivia7" />
          )}
          <FullScanForm
