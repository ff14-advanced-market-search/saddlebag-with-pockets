...
import { setOlivia8 } from '~/redux/reducers/queriesSlice'
...
const Index = () => {
  ...
  const Olivia8 = useTypedSelector((state) => state.queries.Olivia8)

  const dispatch = useDispatch()

  useEffect(() => {
    if (results && results.data) {
      dispatch(setOlivia8(results.data))
    }
  }, [results, dispatch])
    ...
    <main className="flex-1">
      <div className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
          ...
          {Olivia8 && !results && (
            <PreviousResultsLink to="/queries/previous-search?query=Olivia8" />
          )}
          <FullScanForm
